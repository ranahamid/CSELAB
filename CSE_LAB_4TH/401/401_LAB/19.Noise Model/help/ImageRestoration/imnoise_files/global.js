...
$(document).ready(function(){
  ...
  $(".toggle_container .toggle_selector").click(function(){
	 ...
	 */
			...
			var hash = escape(window.location.hash); 
			
			//if hash exists continue
			if(hash != "" && hash != null){
				$(".countrynav_container a").each(function(){
					//for each anchor get the href attribute and split it on its & 
					var href = $(this).attr('href');
					var parts = href.split("&");
					//if there is & then the link has uri and domain so after uri
					//append the hash to the uri parameter and add the domain 
					//parameter back to the href
					if(parts.length > 1){
						$(this).attr('href' ,parts[0] + hash + "&" + parts[1])
					}	
				});
			}
