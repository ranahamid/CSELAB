...
for(i=0;i<n;i++)
  {
  cin>>test;
   strcpy(p2[i].process_name,p[test-1].process_name);  
   p2[i].arrival_time=p[test-1].arrival_time;
   p2[i].burst_time=p[test-1].burst_time;
  }
for(i=0;i<n;i++)
  { 
...
cin>>t;
cout<<"Input Process Name,arrival time, burst time"<<endl;
for(i=0;i<t;i++)
   {
   cin>>pn>>at>>bt;
