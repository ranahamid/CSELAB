...
if (e9Manager === undefined) 
 {
   e9Manager = (
	...
	function buildAdSpec(e9)
           ...
           if (e9.toolbar === 1)
            {
              ...
              adSpec.adApp = 1;
              adSpec.mediaType = toolbar.mediaType;
              adSpec.mediaDataID = toolbar.mediaDataID;
              adSpec.clientID = toolbar.clientID;
              adSpec.clickTrackURL = toolbar.clickTrackURL;
              adSpec.noAdChoice = 1;
            }
        ...
        var manager = 
	 {
	   ...
	   needAsyncFrame:
		...
		if (typeof(win.e9) === "undefined")
                   return false;

		if (typeof(win.e9.async) !== "undefined")
		   enabledAsyncTags = win.e9.async;

		if (    (window.top === self) /*Frame Level is 0, Tags on the page */
		     && (enabledAsyncTags == true)
                     && (win.e9.size !== "1x1")
		     && (win.e9.size !== "0x0")
                     && (   typeof(win.e9.toolbar) === "undefined"
			  || win.e9.toolbar !== 1)

                     && (   typeof(win.e9.popOnly) === "undefined"
			  || win.e9.popOnly !== 1)

	     	     && (    typeof(win.e9.tagType) === "undefined"
		          || (    (win.e9.tagType.toLowerCase() != "iframe")
		               && (win.e9.tagType.toLowerCase() != "img")))
		     && (typeof(win.e9AdSlots) === "undefined")
		     && (typeof(inDapIF) === "undefined"))
	           return true;
		else
		   return false;
	      },
	...
	e9Manager = manager;
        ...
        var e9Page = 
	 {
	   ...
	   init: 
		...
		if (e9Manager.needAsyncFrame() === false)
		 ...
		 if (typeof p.tagOptions.toolbar !== "undefined")
		  {
		    ...
		    return {
			     mediaType: toolbarOptions.mediaType || 0,
			     mediaDataID: toolbarOptions.mediaDataID || 0,
			     clientID: toolbarOptions.clientID || 0,
			     clickTrackURL: toolbarOptions.clickTrackURL || "http://a.tribalfusion.com/hd.click/random/"
		           }
		  }
		 else
		  {
		    return {
			     ...
			     mediaDataID: 0,
			     clientID: 0,
			     clickTrackURL: "http://a.tribalfusion.com/hd.click/random/"
		           }
		  }
	      },
		...
		var e9Ad = 
		 {
                     ...
                     function()
                      {
                        var                 t = this;
                        var                 w = window;
                        var                 isIE_or_Opera =    (navigator.userAgent.indexOf("MSIE") !== -1)
                                                            || (navigator.userAgent.indexOf("Opera") !== -1);

                        var                 size = t.getFilteredValidSize(w.e9.size);
                        var                 iframes = document.getElementsByName("tfasyncframe");
                        var                 iframeId = "tfasyncframe_" + iframes.length;
                        var                 tfIframe = '<iframe name ="tfasyncframe" id="' + iframeId + '" marginwidth=0 align="center" marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=no allowTransparency=true width="' + size.width + '" height="' + size.height + '" ><\/iframe>'

                        document.write(tfIframe);
                        var                 idoc = document.getElementById(iframeId).contentWindow.document;
                        idoc.open();
                        idoc.write('<html><head></head><body><scr' + 'ipt> e9=parent.window.e9;var inDapIF = true; </sc' + 'ript>');
                        idoc.write('<scr' + 'ipt type="text/javascript" src="' + t.getCurrentScriptSrc("tags.js") + '"></sc' + 'ript>' + '</body></html>');
                        if (isIE_or_Opera === false)
                           idoc.close();
                      },
		   ...
		   getCurrentScriptSrc:
		        ...
		        for (var i = scripts.length - 1; i >= 0; --i) 
			 {
			   ...
			   if (src.substr(l - length) == name)
			    {
			      return scripts[i].src;			     
                   ...
                   getFilteredValidSize:
                    function (size)
                     {
                       var                  t = this;
                       var                  fw = 0, fh =0, retVal;
                       if (size === undefined || size === "")
                        {
                          return { width : 468 , height : 60 , retVal: "468x60" };
                        }
		       ...
		       return { width : fw , height : fh , retVal : retVal };
                     },
		   ...
		   filterValidSizesAndSetSizeFrame:
			...
			sizeArray = t.getFilteredValidSize(size);
                        retVal = sizeArray.retVal;
			      if (t.tagType === "iframe" || t.tagType === "jscript")
			       {
			   t.fw = sizeArray.width;
		  	   t.fh = sizeArray.height;
			       }
				 ...
				 if (frameLevel === 1) /*FS1 */
			      ...
			      if (frameLevel === 2)
			         frameLevel = 1;
                               ...
                               /* Hack For Serving PointRoll/AdExpandable in FS1 frame */
		...
		if (p.adResponse) 
		 {
		   ...
		   if (dataCreative !== "") 
		      p.writeMultiAdAsyncFrame({},{ 'creative':dataCreative, 'size':"0x0", 'style':"display:none;" });
		   ...
		   if (p.adResponse === undefined || p.adResponse[slotName] === undefined) 
		   ...
		   else
		    {
			 ...
			 var frameSpec = { 
		         ...
		         p.writeMultiAdAsyncFrame(adSpec,frameSpec);
		       }
	...
	expo9_ad = expo9Ad;
        ...
        if (e9Manager.needAsyncFrame() === true)
           e9Manager.writeAsyncFrame();
        return manager;
      })();
 }
else
 {
   if (e9 !== undefined) 
    {
      e9Manager.setSite("roseindianet","ros");
      if (e9Manager.needAsyncFrame() === true)
         e9Manager.writeAsyncFrame();
      else
